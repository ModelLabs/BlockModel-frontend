      ...
      <el-collapse v-model="activeNames" class="el-collapse">
        ...
        >
          ...
          <el-form 
            ...
            <el-input type="textarea" :rows="3" v-model="dynamicValidateForm_Module.globalVars" class="form-item" size="mini" placeholder="Global Variables"></el-input>
...
export default {
    ...
    submitForm_Token(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //     }
          // }
          this.nodeData.tokens.tokenNames = this.dynamicValidateForm_Token.tokenName;
          this.nodeData.tokens.tokenAmounts = this.dynamicValidateForm_Token.tokenAmount;
        } else {
    ...
    submitForm_Module(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //     }
          // }
          this.nodeData.modules.moduleNames = this.dynamicValidateForm_Module.moduleName;
          this.nodeData.modules.moduleExpressions = this.dynamicValidateForm_Module.moduleExpressions;
          this.nodeData.modules.globalVars = this.dynamicValidateForm_Module.globalVars;
        } else {
          //console.log('error submit!!');
          return false;
        }
  ...
  mounted() {
    for(let i = 0; i < this.nodeData.tokens.tokenNames.length; i++) {
      this.dynamicValidateForm_Token.tokenName.push(this.nodeData.tokens.tokenNames[i]);
      this.dynamicValidateForm_Token.tokenAmount.push(this.nodeData.tokens.tokenAmounts[i]);
    }
    
    for(let i = 0; i < this.nodeData.modules.moduleNames.length; i++) {
      this.dynamicValidateForm_Module.moduleName.push(this.nodeData.modules.moduleNames[i]);
      this.dynamicValidateForm_Module.moduleExpressions.push(this.nodeData.modules.moduleExpressions[i]);
    }

    this.dynamicValidateForm_Module.globalVars = this.nodeData.modules.globalVars;
