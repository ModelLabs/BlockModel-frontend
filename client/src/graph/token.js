...
export class Token {
    ...
    constructor(symbol, totalSupply, allocations, community) {
        ...
        allocations.forEach(element => {
            ...
            // element 取到的是 token 中每个参与方的分配比例 存进分配 Map
            // 各参与方质押数量、增量、活钱数量、增量分别置0存进对应的 Map
        ...
        if (this.communityPolicy.allocations.has("Airdrop") ) {
            // 计算社区空投的金额 累加到社区的 freeMoney 累加到 freeMoneyTotal
        ...
        // 计算社区被分配的金额 累加到分配增量 allocatedAmountDelta
    ...
    run(preNode, curDay) {
        ...
        if (preNode.constructor === Token) {
            ...
            for (let item of this.freeMoneyDelta) {
                // item[0] 是参与方的名字 item[1]是增量数值，加上质押奖励后放入 freeMoneyDelta
                if (preNode.c.has(item[0])) {
